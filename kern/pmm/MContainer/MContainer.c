...
void container_init(unsigned int mbi_addr)
{
     ...
     */
	unsigned int num_pages = get_nps(); // total number of pages
	for(unsigned int page_index = 0; page_index < num_pages; page_index++){
		if(at_is_norm(page_index) > 0 && at_is_allocated(page_index) == 0){
			// found an unallocated page with normal permission
			real_quota++;
		}
	}
...
{
    // TODO
    unsigned int quota = CONTAINER[id].quota;
	unsigned int usage = CONTAINER[id].usage;
	if(quota - usage >= n){
		return 1;
	} else {
		return 0;
